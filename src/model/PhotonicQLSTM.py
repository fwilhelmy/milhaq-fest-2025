"""Photonic Quantum LSTM components using MerLin and Perceval."""
...
# ---------------------------------------------------------------------------
# Photonic QLSTM components (MerLin + Perceval)
# ---------------------------------------------------------------------------
...
def make_photonic_vqc_2n_modes(
    ...
    m = 2 * n_inputs
    circuit = bs_layer_on_pairs(m) // ps_encode_inputs(m, n_inputs) // universal_interferometer(m, "U")
...
class PhotonicQLSTMCell(nn.Module):
    def __init__(
        ...
        self.vqc_i = make_photonic_vqc_2n_modes(n_v, hidden_size, shots, dtype=self._dtype)
        self.vqc_f = make_photonic_vqc_2n_modes(n_v, hidden_size, shots, dtype=self._dtype)
        self.vqc_g = make_photonic_vqc_2n_modes(n_v, hidden_size, shots, dtype=self._dtype)
        self.vqc_o = make_photonic_vqc_2n_modes(n_v, hidden_size, shots, dtype=self._dtype)

        if use_photonic_head:
            self.vqc_head = make_photonic_vqc_2n_modes(hidden_size, output_size, shots, dtype=self._dtype)
        else:
            self.output_proj = nn.Linear(hidden_size, output_size, dtype=self._dtype)
        ...
        parts = [
            ...
            f"    · gate input n = {n_v} -> modes={2 * n_v}, photons={n_v}",
            f"    · measurement={measurement} -> gate output dim={gate_out_dim}",
        ]
        if use_photonic_head:
            parts.append(
                f"  - head: photonic VQC on 2*hidden modes -> modes={2 * hidden_size}, photons={hidden_size}"
            )
        else:
...
class PhotonicQLSTM(BaseModel):
    """Lightning wrapper around the photonic QLSTM cell (MerLin/Perceval)."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = torch.zeros(bsz, self.hidden_size, device=x.device, dtype=self.cell._dtype)
        c = torch.zeros_like(h)
        out = None
        for t in range(timesteps):
            x_t = x[:, t, :].to(self.cell._dtype)
            out, (h, c) = self.cell(x_t, (h, c))
        if out is None:
            raise RuntimeError("PhotonicQLSTM received an empty sequence")
        return out
